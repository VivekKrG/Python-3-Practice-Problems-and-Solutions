...
def main():
		...
		for i in range(8):
			for j in range(8):  # index-1 is parent index(inner index)
				if index - 1 in path_char.get(matrix[i][j], []):
					...
					elif index <= path_size-2 and index < 5:  # path_size - 1:
						for num in range(8):
							x = i + cord[num][0]
							y = j + cord[num][1]
							# print('direction:', num, 'x, y:', x, y)
	...
	# Use inform1 matrix: inform1[][] gives list of all paths up to index 4 -> path is set of coordinates
	# print(inform1)
	for i in range(8):
		for j in range(8):
			for current_path in inform1[i][j]:
...
5 a's
77016 from if count
--- 0.06216263771057129 seconds ---
6 a's
408764 from if count
--- 0.40006208419799805 seconds ---
Better above 6
7 a's
2129440
--- 2.1647493839263916 seconds ---
___
8-as
10899404
...
10899404
--- 10.515458345413208 seconds ---
9 a's
54738536
--- 50.14827847480774 seconds ---
